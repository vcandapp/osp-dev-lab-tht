...
infrared tripleo-overcloud -o overcloud-install.yml --version 17.1 --deployment-files osnet_nm_dev_auto_tht/osp17_ref  --overcloud-templates none --storage-backend lvm --overcloud-ssl no --network-backend vxlan --network-protocol ipv4 --network-bgpvpn no  --network-dvr no  --network-l2gw no --storage-external no --splitstack no --overcloud-debug yes  --overcloud-fencing no  --introspect no  --tagging no --tht-roles yes --deploy yes  --containers True --hybrid instack.json -e provison_virsh_network_name=br-ctlplane  --overcloud-script overcloud_deploy.sh --collect-ansible-facts False --ntp-pool clock.corp.redhat.com
